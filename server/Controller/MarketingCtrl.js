const MarketingTask = require("../Models/MarketingModel");
...
exports.createTask = async (req, res) => {
  try {
    ...
    const newTask = new MarketingTask({
      cat: body.cat,
      sub: body.sub,
      depart: body.depart,
      name: body.name,
...
exports.getAllTasks = async (req, res) => {
  try {
    const tasks = await MarketingTask.find().sort({ createdAt: -1 }); // Newest first
...
exports.deleteTask = async (req, res) => {
  try {
    const deleted = await MarketingTask.findByIdAndDelete(req.params.id);
