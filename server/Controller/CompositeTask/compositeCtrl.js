const CompositeTask = require("../../Models/CompositeTask/CompositeModal");
...
exports.createTask = async (req, res) => {
  try {
    ...
    const newTask = new CompositeTask({
      cat: body.cat,
      sub: body.sub,
      depart: body.depart,
      name: body.name,
...
exports.getAllTasks = async (req, res) => {
  try {
    const tasks = await CompositeTask.find().sort({ createdAt: -1 }); // Newest first
...
exports.getTaskById = async (req, res) => {
  try {
    const task = await CompositeTask.findById(req.params.id);
...
//     const updated = await CompositeTask.findByIdAndUpdate(id, updates, {
//       new: true,
//     });
...
exports.deleteTask = async (req, res) => {
  try {
    const deleted = await CompositeTask.findByIdAndDelete(req.params.id);
