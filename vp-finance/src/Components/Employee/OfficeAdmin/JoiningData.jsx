...
const JoiningData = () => {
  ...
  const handleRankChange = (empId, empType, newRank) => {
    if (window.confirm('Are you sure want to change Rank of Seller Account Holder?')) {
      setEmployees(employees.map(emp => 
        emp.id === empId && emp.type === empType ? {...emp, rank: newRank} : emp
      ));
    }
  };
  ...
  const handleStatusChange = (empId, empType) => {
    if (window.confirm('Are you sure want to change status of employee?')) {
      setEmployees(employees.map(emp => 
        ...
        {...emp, status: emp.status === 'Enabled' ? 'Disabled' : 'Enabled'} : 
        emp
      ));
    }
  };
  ...
  const handleDelete = (empId, empType) => {
    if (window.confirm('Are you sure want to Delete? Once you delete you can not access employee related information!')) {
      setEmployees(employees.filter(emp => !(emp.id === empId && emp.type === empType)));
    }
  };
                    ...
                    <Table responsive bordered hover>
                      <thead>
                        <tr>
                          ...
                          <th><img src="img/Enabled.png" alt="Status" /></th>
                          <th>Action</th>
                        </tr>
                      </thead>
                      <tbody>
                        {currentEntries.map((emp, index) => (
                          <tr key={`${emp.id}-${emp.type}`}>
                            ...
                            <td>
                              <Dropdown>
                                <Dropdown.Toggle variant="danger" size="sm" id="dropdown-rank">
                                  {emp.rank || 'Select Rank'}
                                </Dropdown.Toggle>
                                <Dropdown.Menu>
                                  {[1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5].map(rank => (
                                    <Dropdown.Item 
                                      key={rank}
                                      onClick={() => handleRankChange(emp.id, emp.type, rank)}
                                    >
                                      {rank}
                                    </Dropdown.Item>
                                  ))}
